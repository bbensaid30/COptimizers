...
{
    std::ofstream iterFlux(("Record/SpolyTwo/"+setHyperparameters+"/"+algo+"_"+"iter"+".csv").c_str());
    std::ofstream initFlux(("Record/SpolyTwo/"+setHyperparameters+"/"+algo+"_"+"init"+".csv").c_str());
    std::ofstream iterForwardFlux(("Record/SpolyTwo/"+setHyperparameters+"/"+algo+"_"+"iter_MeanForward"+".csv").c_str());
    ...
    std::vector<std::vector<Sdouble>> proportions_list(nbTirage), iters_list(nbTirage), iters_MeanForward_list(nbTirage);
    std::vector<Sdouble> weights_list(nbTirage), bias_list(nbTirage);
    std::vector<Sdouble> farMin_list(nbTirage), nonMin_list(nbTirage);  
    ...
    #pragma omp parallel
    {
        ...
        std::vector<Sdouble> proportions(4,0.0), iters(4,0.0), distances(4,0.0), iters_MeanForward(4,0.0);
        int numeroPoint;
        Sdouble farMin, nonMin;
    ...
    for(i=0;i<nbTirage;i++)
    {
        ...
        {
            std::copy(weightsInit.begin(),weightsInit.end(),weights.begin()); std::copy(biasInit.begin(),biasInit.end(),bias.begin());
            study = Strain(X,Y,L,nbNeurons,globalIndices,activations,weights,bias,"norme2",famille_algo,algo,eps,maxIter,learning_rate,beta1,beta2,
            batch_size,j,false);
            ...
            if (study["finalGradient"]+std::abs(study["finalGradient"].error)<eps)
            {
                ...
                else{iters[numeroPoint]+=study["iter"]; iters_MeanForward[numeroPoint]+=study["total_iterLoop"]/study["iter"];}
            }
            else
            {
                nonMin++;
        ...
        for(k=0;k<4;k++)
        {
            ...
            proportions[k]/=Sdouble(nbSeeds);
        }
        farMin/=Sdouble(nbSeeds); nonMin/=Sdouble(nbSeeds);
    ...
    if(record)
    {
        for(int i=0; i<nbTirage; i++)
        {
            ...
            initFlux << proportions_list[i][0].number << std::endl;
            initFlux << proportions_list[i][1].number << std::endl;
            initFlux << proportions_list[i][2].number << std::endl;
            initFlux << proportions_list[i][3].number << std::endl;
            initFlux << farMin_list[i].number << std::endl;
            initFlux << nonMin_list[i].number << std::endl;

            iterFlux << weights_list[i].number << std::endl;
            iterFlux << bias_list[i].number << std::endl;
            iterFlux << iters_list[i][0].number << std::endl;
            iterFlux << iters_list[i][1].number << std::endl;
            iterFlux << iters_list[i][2].number << std::endl;
            iterFlux << iters_list[i][3].number << std::endl;

            iterForwardFlux << weights_list[i].number << std::endl;
            iterForwardFlux << bias_list[i].number << std::endl;
            iterForwardFlux << iters_MeanForward_list[i][0].number << std::endl;
            iterForwardFlux << iters_MeanForward_list[i][1].number << std::endl;
            iterForwardFlux << iters_MeanForward_list[i][2].number << std::endl;
            iterForwardFlux << iters_MeanForward_list[i][3].number << std::endl;
        }
    }
...
bool const record, std::string const setHyperparameters)
{
    std::ofstream iterFlux(("Record/SpolyThree/"+setHyperparameters+"/"+algo+"_"+"iter"+".csv").c_str());
    std::ofstream initFlux(("Record/SpolyThree/"+setHyperparameters+"/"+algo+"_"+"init"+".csv").c_str());
    std::ofstream iterForwardFlux(("Record/SpolyThree/"+setHyperparameters+"/"+algo+"_"+"iter_MeanForward"+".csv").c_str());
    if(!iterFlux || !initFlux || !iterForwardFlux){std::cout << "Impossible d'ouvrir un des fichiers en écriture" << std::endl;}
...
bool const record, std::string const setHyperparameters)
{
    std::ofstream iterFlux(("Record/SpolyFive/"+setHyperparameters+"/"+algo+"_"+"iter"+".csv").c_str());
    std::ofstream initFlux(("Record/SpolyFive/"+setHyperparameters+"/"+algo+"_"+"init"+".csv").c_str());
    std::ofstream iterForwardFlux(("Record/SpolyFive/"+setHyperparameters+"/"+algo+"_"+"iter_MeanForward"+".csv").c_str());
    if(!iterFlux || !initFlux || !iterForwardFlux){std::cout << "Impossible d'ouvrir un des fichiers en écriture" << std::endl;}
    ...
    #pragma omp parallel
    {
        ...
        std::map<std::string,Sdouble> study;
        Eigen::SVectorXd currentPoint(2);
        std::vector<Sdouble> proportions(15,0.0), iters(15,0.0), distances(15,0.0), iters_MeanForward(15,0.0);
        int numeroPoint;
        Sdouble farMin, nonMin;
    ...
    if(record)
    {
        for(int i=0; i<nbTirage; i++)
        {
            ...
            for(int k=0; k<15; k++)
            {
                initFlux << proportions_list[i][k].number << std::endl;
            }
            initFlux << farMin_list[i].number << std::endl;
            initFlux << nonMin_list[i].number << std::endl;
            ...
            for(int k=0; k<15; k++)
            {
                iterFlux << iters_list[i][k].number << std::endl;
                iterForwardFlux << iters_MeanForward_list[i][k].number << std::endl;
            }
        }
    }
